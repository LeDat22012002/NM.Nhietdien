...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class NoiHoi_LB1Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public NoiHoi_LB1Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_NoiHoi_LB1")]
        [Authorize]
        public async Task<IActionResult> GetData_NoiHoi_LB1(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<BmLb1>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        [Authorize]
        public IActionResult ExportExcel_NoiHoi_LB1(DateTime? begind, DateTime? endd, int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.BmLb1s
            ...
            var flatData = new List<NoiHoi_LBValiadation>();
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {

                    var prop = typeof(BmLb1).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
                        var value = prop.GetValue(item) as double?;
                        if (value.HasValue)
                        {
                            flatData.Add(new NoiHoi_LBValiadation
                            {
            ...
            string templatePath = Path.Combine(Directory.GetCurrentDirectory(), "App_Data", "BM.15-QT.05.08 Nhat ky theo doi van hanh loc bui khu luu huynhBM2_ 25.09.23.xlsx");
            if (!System.IO.File.Exists(templatePath))
                return BadRequest("Không tìm thấy file mẫu.");

            using var workbook = new XLWorkbook(templatePath);
            var worksheet = workbook.Worksheet("LB"); // Sheet name
            ...
            var fileName = $"NoiHoi_LB1_{DateTime.Now:yyyy-MM-dd}.xlsx";
