...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class NoiHoi_LB9Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public NoiHoi_LB9Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_NoiHoi_LB9")]
        [Authorize]
        public async Task<IActionResult> GetData_NoiHoi_LB9(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<BmLb9>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        [Authorize]
        public IActionResult ExportExcel_NoiHoi_LB9(DateTime? begind, DateTime? endd, int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.BmLb9s
            ...
            var flatData = new List<NoiHoi_LB910Valiadation>();
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {

                    var prop = typeof(BmLb9).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
                        var value = prop.GetValue(item) as double?;
                        if (value.HasValue)
                        {
                            flatData.Add(new NoiHoi_LB910Valiadation
                            {
            ...
            string templatePath = Path.Combine(Directory.GetCurrentDirectory(), "App_Data", "BM.34-QT.05.08 Nhat ky theo doi van hanh loc bui khu luu huynh GD2 BM1.xlsx");
            if (!System.IO.File.Exists(templatePath))
                return BadRequest("Không tìm thấy file mẫu.");

            using var workbook = new XLWorkbook(templatePath);
            var worksheet = workbook.Worksheet("BM1"); // Sheet name
            ...
            var fileName = $"NoiHoi_LB9_{DateTime.Now:yyyy-MM-dd}.xlsx";
