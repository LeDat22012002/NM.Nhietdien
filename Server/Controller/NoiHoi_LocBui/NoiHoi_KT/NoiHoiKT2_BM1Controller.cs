...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class NoiHoiKT2_BM1Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public NoiHoiKT2_BM1Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_NoiHoiKT2_BM1")]
        [Authorize]
        public async Task<IActionResult> GetData_NoiHoiKT2_BM1(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            var query = _context.BmNhkt2Bm1s
            ...
            return Ok(new ApiResponse<List<BmNhkt2Bm1>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        [Authorize]
        public IActionResult ExportExcel_NoiHoiKT2_BM1(DateTime? begind, DateTime? endd, int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.BmNhkt2Bm1s
            ...
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {
                    ...
                    var prop = typeof(BmNhkt2Bm1).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
            ...
            var fileName = $"NoiHoiTK2_BM1_{DateTime.Now:yyyy-MM-dd}.xlsx";
