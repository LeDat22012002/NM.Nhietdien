...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class NoiHoiKT2_BM2Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public NoiHoiKT2_BM2Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_NoiHoiKT2_BM2")]
        [Authorize]
        public async Task<IActionResult> GetData_NoiHoiKT2_BM2(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            var query = _context.BmNhkt2Bm2s
            ...
            return Ok(new ApiResponse<List<BmNhkt2Bm2>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        [Authorize]
        public IActionResult ExportExcel_NoiHoiKT2_BM2(DateTime? begind, DateTime? endd, int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.BmNhkt2Bm2s
            ...
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {
                    ...
                    var prop = typeof(BmNhkt2Bm2).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
                        var value = prop.GetValue(item) as double?;
                        if (value.HasValue)
                        {
                            flatData.Add(new NoiHoiKT_BM2Valiadation
                            {
            ...
            var worksheet = workbook.Worksheet("BM20"); // Sheet name
            ...
            var fileName = $"NoiHoiTK2_BM2_{DateTime.Now:yyyy-MM-dd}.xlsx";
