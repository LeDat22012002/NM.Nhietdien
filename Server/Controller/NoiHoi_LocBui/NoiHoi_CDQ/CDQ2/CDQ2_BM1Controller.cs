...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class CDQ2_BM1Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public CDQ2_BM1Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_CDQ2_BM1")]
        public async Task<IActionResult> GetData_CDQ2_BM1(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<BmCdq2>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        public IActionResult ExportExcel_CDQ2_BM1(DateTime? begind, DateTime? endd , int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.BmCdq2s
            ...
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {
                    ...
                    var prop = typeof(BmCdq2).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
           ...
           var fileName = $"BM1_CDQ2_{DateTime.Now:yyyy-MM-dd}.xlsx";
