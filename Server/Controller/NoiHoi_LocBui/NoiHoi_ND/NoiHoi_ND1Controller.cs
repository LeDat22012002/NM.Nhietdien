...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class NoiHoi_ND1Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public NoiHoi_ND1Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_NoiHoi_ND1")]
        [Authorize]
        public async Task<IActionResult> GetData_NoiHoi_ND1(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<BmNh1>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        [Authorize]
        public IActionResult ExportExcel_NoiHoi_ND1(DateTime? begind, DateTime? endd, int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.BmNh1s
            ...
            var flatData = new List<NoiHoi_NDValiadation>();
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {

                    var prop = typeof(BmNh1).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
                        var value = prop.GetValue(item) as double?;
                        if (value.HasValue)
                        {
                            flatData.Add(new NoiHoi_NDValiadation
                            {
            ...
            string templatePath = Path.Combine(Directory.GetCurrentDirectory(), "App_Data", "BM.14-QT.05.08 Nhat ky van hanh noi hoi nhiet du 25.09.23.xlsx");
            if (!System.IO.File.Exists(templatePath))
                return BadRequest("Không tìm thấy file mẫu.");

            using var workbook = new XLWorkbook(templatePath);
            var worksheet = workbook.Worksheet("Sheet1"); // Sheet name
            ...
            var fileName = $"BM_NHND1_{DateTime.Now:yyyy-MM-dd}.xlsx";
