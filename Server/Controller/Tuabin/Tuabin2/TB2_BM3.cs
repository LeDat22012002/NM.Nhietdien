...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class TB2_BM3Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public TB2_BM3Controller(NMCD2Context context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> GetDataTB2_BM3(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<Tb2Bm3>>
            {
                Status = true,
        ...
        public IActionResult ExportBM3ToExcel(DateTime? begind, DateTime? endd , int turbineIndex = 1)
        {
            ...
            var flatData = new List<TB2_BM3Validation>();
            foreach (var item in data)
            {
                for (int i = 1; i <= 73; i++)
                {
                    ...
                    if (prop != null)
                    {
                        ...
                        if (value.HasValue)
                        {
                            flatData.Add(new TB2_BM3Validation
                            {
