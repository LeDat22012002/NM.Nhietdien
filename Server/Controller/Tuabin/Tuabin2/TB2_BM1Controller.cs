...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class TB2_BM1Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public TB2_BM1Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_TB2_BM1")]
        [Authorize]
        public async Task<IActionResult> GetData_TB2_BM1(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<Tb2Bm1>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        [Authorize]
        public IActionResult ExportExcel_TB2_BM1(DateTime? begind, DateTime? endd, int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.Tb2Bm1s
            ...
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {
                    ...
                    var prop = typeof(Tb2Bm1).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
            ...
            var fileName = $"BM1_TB2_{DateTime.Now:yyyy-MM-dd}.xlsx";
