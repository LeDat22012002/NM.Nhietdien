...
namespace Server.Controllers
{
    ...
    [ApiController]
    public class TB1_BM2Controller : ControllerBase
    {
        private readonly NMCD2Context _context;

        public TB1_BM2Controller(NMCD2Context context)
        {
            _context = context;
        }

        [HttpGet("getData_TB1_BM2")]
        public async Task<IActionResult> GetData_TB1_BM2(DateTime? begind, DateTime? endd, string sortOrder = "desc")
        {
            ...
            return Ok(new ApiResponse<List<Tb1Bm2>>
            {
                Status = true,
        ...
        [HttpGet("exportExcel")]
        public IActionResult ExportExcel_TB1_BM2(DateTime? begind, DateTime? endd , int turbineIndex = 1)
        {
            // 1. Lấy dữ liệu từ database
            var data = _context.Tb1Bm2s
            ...
            foreach (var item in data)
            {
                for (int i = 1; i <= 51; i++)
                {
                    ...
                    var prop = typeof(Tb1Bm2).GetProperty($"Tag{i}");
                    if (prop != null)
                    {
            ...
            string filename = $"BM2_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
